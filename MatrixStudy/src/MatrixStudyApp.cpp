...
class MatrixStudyApp : public AppBasic {
  public:
...
void MatrixStudyApp::prepareSettings( Settings *settings )
{
...
void MatrixStudyApp::mouseDown( MouseEvent event )
{
}

void MatrixStudyApp::keyDown( KeyEvent event )
{
    if( event.getChar() == 'z' ){
		mShowCube = ! mShowCube;
	}
...
void MatrixStudyApp::printMatrix( Matrix44 <float> m)
{
    cout << "\n";
...
Vec3f MatrixStudyApp::getWorldEndPosition( Vec3f localStartPosition, Vec3f localBasisRotation, Vec3f localObjectRotation, Vec3f localEndPosition )
{
    Matrix44 <float> m4;
...
void MatrixStudyApp::draw()
{
    ...
    gl::rotate( mLocalBasisRotation );
        ...
        if (mShowCube) {
            gl::drawCube( mLocalEndPosition, Vec3f(30.0f, 30.0f, 30.0f) );
        }
    ...
    if (!mShowCube) {
        ...
        gl::drawCube( transformedPosition3, Vec3f(30.0f, 30.0f, 30.0f) );
    }
...
CINDER_APP_BASIC( MatrixStudyApp, RendererGl )
