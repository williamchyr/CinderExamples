...
void Turtle::draw()
{
    ...
    // DRAW A CUBE At START POINT
    gl::pushMatrices();
    gl::translate( mWorldStartPosition );
    ci::ColorA color2( CM_RGB, 1.0f, 1.0f, 0.0f );
    glMaterialfv( GL_FRONT, GL_DIFFUSE,	color2 );
    gl::drawCube( Vec3f(0.0f, 0.0f, 0.0f), Vec3f(15.0f, 15.0f, 15.0f) );
    gl::popMatrices();
    
    // DRAW A CUBE AT END POINT
    gl::pushMatrices();
    gl::translate( mWorldEndPosition );
    ci::ColorA color3( CM_RGB, 1.0f, 0.0f, 0.0f );
