...
void Turtle3DApp::setup()
{
    ...
    //TURTLE
    originalPosition.set( 0.0f, 0.0f, 0.0f);
    originalRotation.set( 0.0f, 45.0f, 90.0f);
    originalLength = 300.0f;
    orginalRadius = 20.0f;
    
    firstTurtle.init( originalPosition, Vec3f(0.0f, 0.0f, 0.0f), originalRotation, originalLength, orginalRadius, 1.0f, 0.0f, 1.0f );
    
    mTurtles.push_back( firstTurtle );
    
    direction1.set( 0.0f, 0.0f, 30.0f );
        ...
        */
        if (mTurtles[i].branchNow2 ){
            if ( mTurtles[i].mFinalLength > 2.0f)  {
                Turtle newTurtle;
                newTurtle.init( mTurtles[i].mWorldEndPosition, mTurtles[i].mTotalRotationMatrix, direction1, mTurtles[i].mFinalLength*0.5f, mTurtles[i].mRadius*0.7f, 1.0f, 1.0f, 1.0f);
